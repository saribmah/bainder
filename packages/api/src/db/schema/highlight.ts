...
//
// Position is encoded as a chapter or page reference plus character offsets
// into the canonical text payload — `epub_chapter.html`'s textContent for
// EPUBs, `pdf_page.text` for PDFs. Exactly one of `epub_chapter_order` /
// `pdf_page_number` is set; a CHECK constraint enforces this so a corrupt
// row can't slip in.
//
...
export const highlight = sqliteTable(
  ...
  (table) => [
    ...
    index("highlight_document_page_idx").on(table.documentId, table.pdfPageNumber),
    check(
      "highlight_target_xor",
